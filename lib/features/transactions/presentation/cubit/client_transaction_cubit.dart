...
part 'client_transaction_state.dart';

class ClientTransactionCubit extends Cubit<ClientTransactionState> {
  Timer? _debounceTimer;

  ClientTransactionCubit() : super(ClientTransactionState.initial());
  ...
  Query _buildQuery() {
    Query query = FBFireStore.transactions
        .where('clientId', isEqualTo: state.clientId)
  ...
  Future<void> _performSearch(String searchQuery) async {
    try {
      // Get all transactions for the client first
      final query = _buildQuery();
      final snapshot = await query.get();
  ...
  Future<void> filterByStatus(String status) async {
    emit(state.copyWith(statusFilter: status, currentPage: 1, isLoading: true));
